...
void main(void)
{
	...
	while(1)
	{
		//-----------查询1到20条中的短信,只要有短信就处理------------------------------------------
          ...
          if(i==0)
          {
                   Send_AT_Command(AT_CMGF1,uart_buff,0);
                        //AT+CMGS=   "13818120592"    >
                     strcpy(para_temp,"\"15922260566\"");
					Send_AT_Command(SMS_CMGS,uart_buff,para_temp);
		    	    Send_AT_Command(CMGS_MUB,uart_buff,"SEND: OK!\x1a");
